...
export const joinDuel = functions.https.onRequest(async (req, res) => {
    try {
      ...
      // Wenn keine Warteschlange existiert oder sie leer ist, füge den Spieler zur Liste hinzur
      if (!queueDoc.exists || !queueDoc.data()?.openDuels?.length) {
        await queueRef.set(
          {
            classroomId: classroomId,
            openDuels: FieldValue.arrayUnion(userId), // Nur User-IDs speichern
          },
          { merge: true }
        );
      ...
      const opponentId = openDuels[0]; // Der erste Spieler in der Warteschlange
      await queueRef.update({
        openDuels: FieldValue.arrayRemove(opponentId), // Gegner aus der Warteschlange entfernen
      });
      ...
      const duelRef = await db.collection("duels").add({
        ...
        score: { [opponentId]: 0, [userId]: 0 },
        rounds,
        createdAt: FieldValue.serverTimestamp(),
      });
  
      // Spieler benachrichtigen
      await sendNotification(opponentId, "Du bist dran!", "Das Duell wurde gestartet. Beantworte deine erste Frage.", {
        duelId: duelRef.id,
      });
